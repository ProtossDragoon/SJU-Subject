...
//question method
int set_members_byLinkedList(gr* A, e element) {

	//그룹 A에 element 멤버가 있으면 참값을 반환한다.

	if (set_isEmpty_byLinkedList(A)) {
		return 0;
	}
	nd* node;

	if (A->grouplist->existheader == 1) {
		node = A->grouplist->header->next_node;
	}
	else {
		node = A->grouplist->noheader;
	}

	while (1) {

		int intelem = node->element->intelem;

		if (intelem > element.intelem) {
			return 0;
		}
		else if (intelem < element.intelem) {
...
int set_subset_byLinkedList(gr* A, gr* B, int * remember_me_please) {
	//집합 a가 집합 b의 부분집합인지를 반환

	// 집합 A가 B의 부분집합인지 알아낸다.

	if (set_isEmpty_byLinkedList(A)) {
		return 1;
	}
	//A는 놔두고, B를 비교해가며 A를 제거하면 됨.
	...
	while (1) {
		...
		if (set_members_byLinkedList(B, *(node->element))) {
			node = node->next_node;
			set_removeFirstElement_byLinkedList(A);
			if (set_isEmpty_byLinkedList(A)) return 1;
		}
		else {
			*remember_me_please = set_removeFirstElement_byLinkedList(A).intelem;
			return 0;
		}
...
int main() {
	...
	int* remember_me_please;
	remember_me_please = (int*)malloc(sizeof(int*) * 1);
	...
	if (set_subset_byLinkedList(group1, group2, remember_me_please)) {
		printf("0");
	}
	else {
		printf("%d", *remember_me_please);
	}
