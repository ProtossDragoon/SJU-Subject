...
/*
문자를 넣을 수 있는 stack을 만들어 
아래의 조건과 같은 과정을 거치는 프로그램을 작성하자.

1. 소문자 알파벳으로 구성된 문자열을 입력받아
각각의 문자를 stack A 에 넣는다.
예>  fkdqx    => x (top) -> q -> d -> k -> f

2. 보조 stack을 만들어 stack A에 있는 문자를 정렬된 순서로 쌓는다.
top에서 bottom 으로 오름차순.
예> d(top) ->f -> k -> q -> x

3. stack에 있는 것을 top에서부터 뽑아 하나씩 출력한다.
예> dfkqx

조건! Stack을 Queue 두개로 구현하여 만들자.
