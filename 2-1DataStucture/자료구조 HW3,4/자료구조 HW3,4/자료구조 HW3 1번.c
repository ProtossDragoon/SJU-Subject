...
*/
	...
	queue1 = initQue_linkedlist(SINGLE_LINKED_LIST);
	queue2 = initQue_linkedlist(SINGLE_LINKED_LIST);
	...
	//string 1과 2의 길이가 같다는 가정
	cp = string1;
	while (*cp != 0) {
		
		element = setElement(0, *cp);
		elem = enqueue_linkedlist(queue1, element);
		cp++;
	...
	while (*cp != 0) {
		...
		element = setElement(0, *cp);
		elem = enqueue_linkedlist(queue2, element);
		cp++;
