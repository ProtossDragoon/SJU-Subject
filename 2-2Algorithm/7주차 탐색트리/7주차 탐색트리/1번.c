...
<!> heap sort 에 관하여
	...
	<--- 개정 --->
		- reduceExternal 함수를 구축하지 않고 바보짓했음.
		- reduceExternal 함수 10/21 에 추가, 일부 함수들만 reduce external 함수 사용함.

<!> SearchTree
	- 적절한 e 가 없을 경우에도 e 를 반환하나 e.charelement 의 값이 '_' 입니다.
...
treenode* sibling_treenode_binarytree(treenode* node) {

	if (isRoot_treenode_binarytree(node)) return;

	treenode* np;
	np = node->parent;

	if (np->leftchild == node) {

		return np->rightchild;
	}

	if (np->rightchild == node) {
...
void visitPreOrder_treenode_binarytree(treenode* node) {
...
void preOrder_treenode_binarytree(treenode* node) {

	visitPreOrder_treenode_binarytree(node);
	if (!isExternal_treenode_binarytree(node)) {
		preOrder_treenode_binarytree(node->leftchild);
		preOrder_treenode_binarytree(node->rightchild);
	}
}
...
void postOrder_treenode_binarytree(treenode* node) {
...
void inOrder_treenode_binarytree(treenode* node) {
...
void levelOrder_treenode_binarytree(treenode* node) {
...
treenode* inOrderSuccessor_treenode_binarytree(treenode* node) {

	treenode* np = NULL;

	if (!isExternal_treenode_binarytree(node)) {
		np = rightChild_treenode_binarytree(node);
		while (!isExternal_treenode_binarytree(np)) {
			np = leftChild_treenode_binarytree(np);
		}
		return np;
	}

	np = parent_treenode_binarytree(node);

	while (node == rightChild_treenode_binarytree(np)) {

		if (isRoot_treenode_binarytree(np)) {
			invalidNodeException();
			return np;
		}

		node = np;
		np = parent_treenode_binarytree(np);
...
#define UPSMALL 0	//위가 작은것
#define UPBIG 1		//위가 큰것
#define UPPER 0		//상향식 힙
#define DOWNER 1	//일반 힙

//heap ADT - 1. Linked List
...
element reduceExternal_treenode_binarySearchtree(treenode* externalnode) {

	if (!isExternal_treenode_binarytree(externalnode)) return;

	if (isRoot_treenode_binarytree(externalnode->parent)) return;

	treenode* removenode;
	removenode = externalnode->parent;

	treenode* np;
	np = sibling_treenode_binarytree(externalnode);


	np->parent = removenode->parent;


	if (removenode->parent->leftchild == removenode) { 
		//내가 지우고자 하는 노드가 조상노드의 왼쪽자식이면
		removenode->parent->leftchild = np;
	}
	else { 
		// 내가 지우고자 하는 노드가 조상노드의 오른쪽자식이면
		removenode->parent->rightchild = np;
	}

	element e;
	e = remove_treenode_binarytree(externalnode->parent);
	remove_treenode_binarytree(externalnode);

	return e;

}
void noSuchKey_treenode_binarySearchtree(int insert_or_search) {
	if (insert_or_search == INSERT) {
		printf("");
	}
	else if (insert_or_search == RELEASE) {
		printf("X\n");
	}
	else if (insert_or_search == SEARCH){
		printf("");
	}
	else
		printf("");
}
treenode* findNode_treenode_binarySearchtree(treenode* node, element e, int insert_or_search) {

	if (isExternal_treenode_binarytree(node)) {
		noSuchKey_treenode_binarySearchtree(insert_or_search);
		return node;
	}

	if (node->e.intelement == e.intelement) {
		return node;
	}

	else if (node->e.intelement < e.intelement) {
		findNode_treenode_binarySearchtree(rightChild_treenode_binarytree(node), e, insert_or_search);
	}
	else {
		findNode_treenode_binarySearchtree(leftChild_treenode_binarytree(node), e, insert_or_search);
	}

}

//element e 에 저장된 intelement 를 key 로 삼아 비교하고, 전체 원소 element e 를 return. input time 에는 최종노드 return
element treeSearch_treenode_binarySearchtree(binarytree* tree, element e, int insert_or_search) {
	treenode* node;
	node = findNode_treenode_binarySearchtree(tree->root, e, insert_or_search);
	if (isExternal_treenode_binarytree(node)) {
		element e;
		init_element(&e);
		e.charelement = '_';
...
void insertItem_treenode_binarySearchtree(binarytree* tree, element e) {

	treenode* node = NULL;
	node = findNode_treenode_binarySearchtree(tree->root, e, INSERT);
	if (!isExternal_treenode_binarytree(node)) {
		return;
	}
	else {
...
element removeElement_treenode_binarySearchtree(binarytree* tree, element key) {

	element e;
	treenode* node = NULL;
	
	node = findNode_treenode_binarySearchtree(tree->root, key, SEARCH);
	if (isExternal_treenode_binarytree(node)) {
		noSuchKey_treenode_binarySearchtree(RELEASE);
		element e;
		init_element(&e);
		e.charelement = '_';
		return e;
	...
	treenode* np;
	np = node->leftchild;
	if (!isExternal_treenode_binarytree(node->leftchild)) {
		np = node->rightchild;
	}

	if (isExternal_treenode_binarytree(np)) {
		return reduceExternal_treenode_binarySearchtree(np);
	}



	//double
	treenode* inordersucc_internal = NULL;
	treenode* inordersucc_external = NULL;
	inordersucc_external = inOrderSuccessor_treenode_binarytree(node);
	inordersucc_internal = inordersucc_external->parent;

	e = node->e;
	node->e = inordersucc_internal->e;
	reduceExternal_treenode_binarySearchtree(inordersucc_external);
...
int main() {
	...
	while (1) {
		...
		if (input == 'i' || input == 'I') {
			...
			insertItem_treenode_binarySearchtree(tree, tmpelement);
		}
		
		else if (input == 's' || input == 'S') {//search
			scanf("%d", &key);
			//set datatype element (init)
			tmpelement.intelement = key;
			tmpelement.charelement = '1';
			...
			element e;
			e = treeSearch_treenode_binarySearchtree(tree, tmpelement, RELEASE);
			if (e.charelement != '_')
				printf("%d\n", treeSearch_treenode_binarySearchtree(tree, tmpelement, RELEASE).intelement);
			...
			e = treeSearch_treenode_binarySearchtree(tree, tmpelement, RELEASE);
			if (e.charelement != '_')
				printf("%d\n", removeElement_treenode_binarySearchtree(tree, tmpelement).intelement);
		}
		else if (input == 'p' || input == 'P') {//print
			preOrder_treenode_binarytree(tree->root);
			printf("\n");
//			inOrder_treenode_binarytree(tree->root);
//			printf("\n");
		}
